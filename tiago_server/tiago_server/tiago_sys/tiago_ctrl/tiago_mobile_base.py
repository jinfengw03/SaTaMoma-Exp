...
class TiagoBase:
    ...
    def get_delta_pose(self):
        ...
        return np.array([delta_pos[0], delta_pos[1], delta_euler[2]]) # [x, y ,rz]
...
class TiagoBaseVelocityControl(TiagoBase):
    ...
    def setup_actors(self):
        ...
        self.lin_scale = 0.2 # default: 0.2
        self.ang_scale = 0.2 # default: 0.2
    ...
    def step(self, velocity):
        ...
        if self.base_enabled:
            ...
            lin_cmd[:2] = velocity[:2] # x, y | only x functional for tiago | x(forward-and-backward)
            lin_cmd = self.lin_scale * lin_cmd #/np.linalg.norm(lin_cmd)
            ## [angle command]
            ang_cmd = np.zeros(3)
            # if abs(velocity[2]) > 0.25:
            ang_cmd[2] = velocity[2] # rz
            ang_cmd = self.ang_scale * ang_cmd #/np.linalg.norm(ang_cmd)
            self.base_writer.write(create_twist_command(lin_cmd, ang_cmd))

class TiagoKeyboardTeleop:

    def __init__(self):
        self.controller = TiagoBaseVelocityControl(base_enabled=True)
        self.velocity = np.zeros(3)  # [vx, vy, wz]
        self.key_mapping = {
            'w': (0.3, 0.0, 0.0),   # forward
            's': (-0.3, 0.0, 0.0),  # backward
            'a': (0.0, 0.0, 0.3),   # rotate left
            'd': (0.0, 0.0, -0.3),  # rotate right
            'q': (0.0, 0.0, 0.0),   # stop
        }
    ...
    def on_press(self, key):
        try:
            if key.char in self.key_mapping:
                vx, vy, wz = self.key_mapping[key.char]
    ...
    def run(self):
        rate = rospy.Rate(500)  # 10Hz loop
        while not rospy.is_shutdown():
            self.controller.step(self.velocity)
            rate.sleep()

if __name__ == '__main__':
    rospy.init_node('tiago_mobile_base_check')
    teleop = TiagoKeyboardTeleop()

...
from tiago_server.tiago_sys.utils.ros_utils import Publisher, Listener
...
class TiagoTorso:
    ...
    def step(self, position_delta):
        if self.torso_enabled and (position_delta is not None):
            ...
            self.torso_writer.write(self.create_torso_command(goal_position)) # [absolute control]
    ...
    def reset(self, abs_pos):
        if abs_pos is not None:
            self.torso_writer.write(self.create_torso_command(abs_pos)) # [absolute control]


if __name__ == "__main__":

    rospy.init_node('tiago_torso_check')
    torso = TiagoTorso(torso_enabled=True)

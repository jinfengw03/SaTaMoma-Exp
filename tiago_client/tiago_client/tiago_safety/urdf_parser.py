...
# Compatibility for newer NumPy versions where np.float is removed
...
def _rewrite_package_uris(filename, package_map):
    """Return a temp file path with package:// URIs rewritten to absolute paths."""
    with open(filename, 'r') as f:
        content = f.read()
    for pkg, path in package_map.items():
        content = content.replace(f"package://{pkg}/", path.rstrip('/') + '/')
...
def gs_parse_urdf(filename, merge_fixed=True, links_to_keep=[], fixed_base=True):
    # Build a package map so package:// URIs resolve to local folders under the URDF directory
    ...
    # urdfpy versions differ; if package_map kwarg unsupported, rewrite the URDF.
    try:
        robot = URDF.load(filename, package_map=package_map)
    except TypeError:
        rewritten = _rewrite_package_uris(filename, package_map)
        robot = URDF.load(rewritten)
